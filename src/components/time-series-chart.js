...
export class TimeSeriesChart {
  ...
  _drawUnifiedBackground(ctx, marginLeft, plotWidth, plotHeight, marginTop, plotBottom, tTemp, tSpeed, tDir, tSpeedMin, tSpeedMedian, tSpeedMax) {
    ...
    const legendItems = isShortScale ? [
      ...
      { label: '風向 [deg]', color: tDir.color, type: 'diamond' }
    ] : [
      { label: '気温 [℃]', color: tTemp.color, type: 'line' },
    ...
    measuredItems.forEach((item) => {
      if (item.type === 'line') {
        ...
        ctx.stroke();
        ctx.setLineDash([]);
      } else if (item.type === 'diamond' || item.type === 'dot') {
        ctx.fillStyle = item.color;
        const cx = curX + iconWidth / 2;
        const cy = legendY - 4;
        ctx.beginPath();
  ...
  /**
   * 風向プロットの描画（縦長ひし形プロット / 散布図、ラインで繋がない）
   * 同一値が連続した際も横に連なって直線に見えず、必ず左右に隙間ができて点線に見えるよう、
   * プロット形状を縦長ひし形とする（左右半幅は隣接間隔stepXの32%以下・上限1.0px、上下半高2.4px）。
   * @private
   */
  _drawDirectionPlot(ctx, tDir, marginLeft, plotWidth, plotHeight, plotBottom) {
    ctx.fillStyle = tDir.color;
    const stepX = plotWidth / (PLOT_COUNT - 1);
    // 左右に必ず隙間ができるよう、半幅は隣接間隔の32%（最大1.0px）に制限
    const rx = Math.min(1.0, stepX * 0.32);
    // 見やすい大きさを維持するため、上下方向の半高は 2.4px（全高 4.8px）
    const ry = 2.4;
